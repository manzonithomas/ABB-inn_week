...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validazione
    if ($t['macchinario_id'] === 0)    $errors[] = 'Seleziona un macchinario.';
    if ($t['data_inserimento'] === '') $errors[] = 'La data di inserimento è obbligatoria.';
    if ($t['data_scadenza'] === '')    $errors[] = 'La data di scadenza è obbligatoria.';
    if ($t['tecnico'] === '')          $errors[] = 'Il nome del tecnico è obbligatorio.';
    ...
    if (empty($errors)) {
        if ($is_new) {
            ...
            ");
            $stmt->execute([
                $t['macchinario_id'], $t['data_inserimento'], $t['data_scadenza'],
                $t['tecnico'], $t['ente_certificatore'], $t['numero_certificato'],
                $t['esito'], $t['note'], $pdf_path
            ]);
    ...
    <div class="page-card">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select id="macchinario_id" name="macchinario_id" class="form-control" required onchange="calcolaScadenza()">
                    <option value="">— Seleziona macchinario —</option>
                    <?php
                    $cur_rep = '';
                    foreach ($macchinari as $mac):
                        if ($mac['reparto_nome'] !== $cur_rep) {
                            if ($cur_rep !== '') echo '</optgroup>';
                            echo '<optgroup label="' . e($mac['reparto_nome']) . '">';
                            $cur_rep = $mac['reparto_nome'];
                        }
                    ?>
                        <option value="<?= $mac['id'] ?>"
                            data-intervallo="<?= /* ci serve per calcolo scadenza automatico */
                                $db->prepare("SELECT intervallo_mesi FROM macchinari WHERE id=?")
                                   ->execute([$mac['id']]) ?>"
                            <?= (string)$t['macchinario_id'] === (string)$mac['id'] ? 'selected' : '' ?>>
                            <?= e($mac['nome']) ?> — <?= e($mac['codice_seriale']) ?>
                        </option>
                    <?php endforeach; if ($cur_rep) echo '</optgroup>'; ?>
                </select>
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <div class="form-hint" id="scadenza-hint">Calcolata automaticamente in base all'intervallo del macchinario.</div>
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="numero_certificato" name="numero_certificato" class="form-control"
                           value="<?= e($t['numero_certificato'] ?? '') ?>"
                           placeholder="es. 2024/TAR/001"
                ...
                <div class="form-group">
                    ...
                    <select id="esito" name="esito" class="form-control">
                        ...
                        <option value="non_conforme" <?= $t['esito'] === 'non_conforme' ? 'selected' : '' ?>>✕ Non conforme</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <?php if (!$is_new && $t['pdf_path']): ?>
                    <div style="margin-bottom:10px; padding:10px 14px; background:#fafafa; border:1px solid #e0e0e0; display:flex; align-items:center; gap:10px;">
                        <i class="fa fa-file-pdf" style="color:var(--red); font-size:1.2rem;"></i>
                        <span style="font-size:.88rem;">PDF attuale:
                            <a href="<?= BASE_URL ?>/<?= e($t['pdf_path']) ?>" target="_blank" style="color:var(--red); font-weight:700;">
                                Visualizza &nearr;
                            </a>
                        </span>
                    </div>
                ...
                <input type="file" id="pdf" name="pdf" class="form-control"
                       accept="application/pdf" <?= $is_new ? 'required' : '' ?>>
